...
try {
  await Deno.remove("/tmp/test.db");
} catch {}

const adapter = t.sqlite({ url: "file:/tmp/test.db" });
...
Deno.test("crud", async ({ step }) => {
  ...
  await step("delete many", async () => {
    ...
    assertEquals(
      deleted,
      newRows.sort((a, b) => a.number - b.number),
    );
  });
...
Deno.test("execute function tests", async ({ step }) => {
  await step("create table using execute", async () => {
  ...
  await step("insert with positional parameters", async () => {
    const result = await adapter.execute(
      "INSERT INTO users (id, name, age) VALUES (?, ?, ?) RETURNING *",
      [1, "Alice", 30],
    );
  ...
  await step("insert with named parameters", async () => {
    const result = await adapter.execute(
      "INSERT INTO users (id, name, age) VALUES (:id, :name, :age) RETURNING *",
  ...
  await step("select data", async () => {
    ...
    assertEquals(result[1].age, 25);
  });

  await step("select with parameters", async () => {
    const result = await adapter.execute(
  ...
  await step("update data", async () => {
    const result = await adapter.execute(
      "UPDATE users SET age = :age WHERE id = :id RETURNING *",
      { id: 2, age: 26 },
    );
    assertEquals(result.length, 1);
    assertEquals(result[0].id, 2);
    assertEquals(result[0].name, "Bob");
    assertEquals(result[0].age, 26);
  });

  await step("delete data", async () => {
    const result = await adapter.execute(
  ...
  await step("verify final state", async () => {
